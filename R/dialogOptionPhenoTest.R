...
dialogOptionPhenoTest <- function(eSet){
    ...
    myGlobalEnv$ttPheno <- tktoplevel()
    #tkwm.geometry(myGlobalEnv$ttPheno,"180x250")
    
    tktitle(myGlobalEnv$ttPheno) <- paste("Associate between a list of variable and the gene expression")
    
    
    # Take LABELS in myGlobalEnv$ttPheno
    label1 <- tklabel(myGlobalEnv$ttPheno, text= "Survival\n Event(Living/Deceased) \n and Time (Months)")
    label2 <- tklabel(myGlobalEnv$ttPheno,text="Categorical \n select non numeric \n variable")
    label3 <- tklabel(myGlobalEnv$ttPheno, text= "Continuous \n select numeric variable")
    label4 <- tklabel(myGlobalEnv$ttPheno,text="p.adjust.method")
    #tkgrid(tklabel0,tklabel1, columnspan=2, pady = 10)
    
    
    yscr1 <- tkscrollbar(myGlobalEnv$ttPheno, repeatinterval=2,
                         command=function(...)tkyview(tl1,...))
    xscr1 <- tkscrollbar(myGlobalEnv$ttPheno, repeatinterval=2,orient="horizontal",
                         command=function(...)tkxview(tl1,...))
    
    xscr1Info <- tkscrollbar(myGlobalEnv$ttPheno, repeatinterval=2,orient="horizontal",
                             command=function(...)tkxview(tl1info,...))
    
    yscr2 <- tkscrollbar(myGlobalEnv$ttPheno, repeatinterval=2,
                         command=function(...)tkyview(tl2,...))
    xscr2 <- tkscrollbar(myGlobalEnv$ttPheno, repeatinterval=2,orient="horizontal",
                         command=function(...)tkxview(tl2,...))
    xscr2Info <- tkscrollbar(myGlobalEnv$ttPheno, repeatinterval=2,orient="horizontal",
                             command=function(...)tkxview(tl2info,...))
    
    yscr3 <- tkscrollbar(myGlobalEnv$ttPheno, repeatinterval=2,
                         command=function(...)tkyview(tl3,...))
    xscr3 <- tkscrollbar(myGlobalEnv$ttPheno, repeatinterval=2,orient="horizontal",
                         command=function(...)tkxview(tl3,...))
    xscr3Info <- tkscrollbar(myGlobalEnv$ttPheno, repeatinterval=2,orient="horizontal",
                             command=function(...)tkxview(tl3info,...))
    yscr4 <- tkscrollbar(myGlobalEnv$ttPheno, repeatinterval=2,
                         command=function(...)tkyview(tl4,...))
    xscr4 <- tkscrollbar(myGlobalEnv$ttPheno, repeatinterval=2,orient="horizontal",
                         command=function(...)tkxview(tl4,...))
    xscr4Info <- tkscrollbar(myGlobalEnv$ttPheno, repeatinterval=2,orient="horizontal",
                             command=function(...)tkxview(tl4info,...))  
    
    
    tl1<-tklistbox(myGlobalEnv$ttPheno,height=5, width= 20 ,selectmode="multiple",xscrollcommand=function(...)tkset(xscr1,...),yscrollcommand=function(...)tkset(yscr1,...),background="white")
    tl1info<-tklistbox(myGlobalEnv$ttPheno,height=1, width= 20,selectmode="single",xscrollcommand=function(...)tkset(xscr1Info,...),background="white")
    tkconfigure(tl1info, foreground="blue", font=police)
    
    tl2<-tklistbox(myGlobalEnv$ttPheno,height=5, width= 20 ,selectmode="single",xscrollcommand=function(...)tkset(xscr2,...),yscrollcommand=function(...)tkset(yscr2,...),background="white")
    tl2info<-tklistbox(myGlobalEnv$ttPheno,height=1, width= 20,selectmode="single",xscrollcommand=function(...)tkset(xscr2Info,...),background="white")
    tkconfigure(tl2info, foreground="blue", font=police)
    
    tl3<-tklistbox(myGlobalEnv$ttPheno,height=5, width= 20 ,selectmode="single",xscrollcommand=function(...)tkset(xscr3,...),yscrollcommand=function(...)tkset(yscr3,...),background="white")
    tl3info<-tklistbox(myGlobalEnv$ttPheno,height=1, width= 20,selectmode="single",xscrollcommand=function(...)tkset(xscr3Info,...),background="white")
    tkconfigure(tl3info, foreground="blue", font=police)
    
    tl4<-tklistbox(myGlobalEnv$ttPheno,height=5, width= 20 ,selectmode="single",xscrollcommand=function(...)tkset(xscr4,...),yscrollcommand=function(...)tkset(yscr4,...),background="white")
    tl4info<-tklistbox(myGlobalEnv$ttPheno,height=1, width= 20,selectmode="single",xscrollcommand=function(...)tkset(xscr4Info,...),background="white")
    ...
    {  
        ...
        if (lcurselectSurvival !=2){
            ...
            tkinsert(tl1info,"end",myGlobalEnv$EventTime)
            tkfocus()
        }
    ...
    Survival.but <-tkbutton(myGlobalEnv$ttPheno,text="select",command=loadSurvival)
    ...
    Categorical.but <-tkbutton(myGlobalEnv$ttPheno,text="select",command=loadCategorical)
    ...
    Continuous.but <-tkbutton(myGlobalEnv$ttPheno,text="select",command=loadContinuous)
    ...
    {  
        ...
        if (lcurselectp.adjust ==0){
            ...
            myGlobalEnv$p.adjustChoice <- p.adjustMethod[curselectp.adjust]
            tkfocus()
        }
    ...
    p.adjust.but <-tkbutton(myGlobalEnv$ttPheno,text="select",command=loadp.adjust.Method)
